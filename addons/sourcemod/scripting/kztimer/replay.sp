...
{
	if(client < 1 || client > MaxClients || !IsClientInGame(client) || g_hRecording[client] != INVALID_HANDLE || g_hBotMimicsRecord[client] != INVALID_HANDLE || IsFakeClient(client))
		return;
...
public LoadReplayPro()
{
	...
	{		
		PlayRecord(g_iBot,0);
		SetEntityRenderColor(g_iBot, 0, 0, 255, 50);
...
public LoadReplayTp()
{
	...
	{			
		...
		SetEntityRenderColor(g_iBot2, 255, 127, 59, 50);
		decl String:szSkin[256];
		decl String:szArm[256];
		Format(szSkin, 256, g_sReplayBotPlayerModel);
		Format(szArm, 256, g_sReplayBotArmModel);
